...
class TrafficModel:
    ...
    def __init__(self):
        ...
        self.params = {}
        self.params['N'] = 50  # number of cells
        self.params['dx'] = 50.  # size of a cell in meters
        self.params['dt'] = 0.02  # time step
        self.params['v_M'] = 120e3 / 60  # 120 km/h in m/min
        self.params['rho_M'] = 0.3  # in cars/m

        self.u = np.zeros((1, self.params['N']))  # N cells
        self.u[0] = np.array(self.params['N'] * [0.1])
    ...
    def propagate_one_step(self, u_in, bc='periodic', rho_in=None):
        ...
        """
        c = self.params['v_M'] * self.params['dt']/self.params['dx']  # much used coeff
        M = np.eye(self.params['N']) * (1 - c) + lower_diagonal_matrix(self.params['N']) * c
        bc_correction = np.zeros(self.params['N'])  # non zero for non periodic bc
        if bc == 'periodic':
        ...
        u_result = M@u_in  +  (2*c/self.params['rho_M']) * u_in**2\
            + (-2*c/self.params['rho_M']) * u_in * u_offset + bc_correction
